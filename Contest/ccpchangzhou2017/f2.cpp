...
void get_num(int k1,int k2,int k3,int k4)
{
	...
	_ = 0;
	vec.push_back(0);
	for(int i = 1;i<=wz[1].num;i++)num[_++] = s[i];
	num[_] = '\0';vec.push_back(_atoi(num));
	_ = 0;
	for(int i = wz[1].num+1;i<=wz[2].num;i++)num[_++] = s[i];
	num[_] = '\0';vec.push_back(_atoi(num));
	_ = 0;
	for(int i =wz[2].num+1;i<=wz[3].num;i++) num[_++] = s[i];
	num[_] = '\0';vec.push_back(_atoi(num));
	_ = 0;
	for(int i =wz[3].num+1;i<=wz[4].num;i++) num[_++] = s[i];
	num[_] = '\0';vec.push_back(_atoi(num));
	_ = 0;
	for(int i = wz[4].num+1;i<=n;i++) num[_++] = s[i];
	num[_] = '\0';vec.push_back(_atoi(num));
	sort(wz+1,wz+5,cmp);
	cout<<"numover"<<endl;
}
